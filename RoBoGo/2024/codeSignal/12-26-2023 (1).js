...
function solution(queryType, query) {
    ...
    const performInsert = (x, y) => {
        customHash[x] = y;
    };

    const performAddToKey = (x) => {
        let updatedHash = {};
        for (let key in customHash) {
            updatedHash[parseInt(key) + x] = customHash[key];
        }
        customHash = updatedHash;
    };

    const performAddToValue = (y) => {
        for (let key in customHash) {
            customHash[key] += y;
        }
    };
    ...
    const performGet = (x) => {
        result += customHash[x] || 0;
    };

    for (let i = 0; i < queryType.length; i++) {
        switch (queryType[i]) {
            case 'insert':
                performInsert(query[i][0], query[i][1]);
                break;
            case 'addToKey':
                performAddToKey(query[i][0]);
                break;
            case 'addToValue':
                performAddToValue(query[i][0]);
                break;
            case 'get':
                performGet(query[i][0]);
                break;
        }
    }
...
// Test cases
console.log(solution(["insert", "insert", "addToValue", "addToKey", "get"], [[1, 2], [2, 3], [2], [1], [3]])); // Output: 5
console.log(solution(["insert", "addToValue", "get", "insert", "addToKey", "addToValue", "get"], [[1, 2], [2], [1], [2, 3], [1], [-1], [3]])); // Output: 6
